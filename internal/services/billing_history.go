...
// Service provides billing history data from ClickHouse
type Service struct {
	clickHouseClient *analytics.ClickHouseClient
}

// NewService creates a new billing history service
func NewService(clickHouseClient *analytics.ClickHouseClient) *Service {
	return &Service{
...
func (s *Service) GetPaymentHistory(ctx context.Context, userID uuid.UUID, filter *BillingHistoryFilter, page, pageSize int) ([]BillingEvent, int64, error) {
...
func (s *Service) GetSubscriptionHistory(ctx context.Context, userID uuid.UUID, filter *BillingHistoryFilter) ([]SubscriptionEvent, error) {
	if s.clickHouseClient == nil {
