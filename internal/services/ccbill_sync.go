...
func NewCCBillSyncService(userService *UserService, subscriptionService *SubscriptionService, productService *ProductService, priceService *PriceService, notificationService *NotificationQueueService) *CCBillSyncService {
...
func (s *CCBillSyncService) processRecord(ctx context.Context, record ccbill.CCBillRecord, dryRun bool) SyncOperation {
	...
	user, err := s.UserService.GetGoTrueUserByEmail(ctx, record.Email)
	if err != nil {
	...
	existingSubscription, err := s.SubscriptionService.GetByUserID(ctx, user.ID)
	if err != nil {
...
func (s *CCBillSyncService) logSyncResults(ctx context.Context, result *SyncResult, dryRun bool) {
	...
	if !dryRun && s.NotificationService != nil {
		...
		if err := s.NotificationService.Create(ctx, summaryNotification); err != nil {
