...
func (s *EntitlementService) AppendEntitlementDays(ctx context.Context, userID, entitlement string, days int, sourceType models.EntitlementSourceType, subscriptionID, paymentID *uuid.UUID) (*models.Entitlement, error) {
    if days <= 0 { return nil, fmt.Errorf("days must be > 0") }
    now := time.Now()
    // If an active indefinite window exists, do nothing.
    {
        ...
        if exists { return nil, fmt.Errorf("cannot append entitlement while subscription entitlement is active") }
    }
    ...
    return s.GrantWindow(ctx, userID, entitlement, start, &end, sourceType, subscriptionID, paymentID)
}
...
func (s *EntitlementService) GrantWindow(ctx context.Context, userID, entitlement string, startAt time.Time, endAt *time.Time, sourceType models.EntitlementSourceType, subscriptionID, paymentID *uuid.UUID) (*models.Entitlement, error) {
