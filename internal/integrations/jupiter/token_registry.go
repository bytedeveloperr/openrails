...
// ToTokenConfigMap converts the registry to the legacy TokenConfig map format.
// This is useful for backwards compatibility with existing code.
func (r *TokenRegistry) ToTokenConfigMap(isDevnet bool) map[string]struct {
	Mint        string
	MainnetMint string
	Symbol      string
	Name        string
	...
	for symbol, token := range r.tokens {
		...
		result[symbol] = struct {
			...
			Enabled:     true,
			Price:       0, // Prices are fetched at runtime
		}
	}
